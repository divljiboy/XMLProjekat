            ...
            <xsl:when test="@ref_akt">
                <xsl:choose>
                    ...
                    <xsl:when test="@ref_tacka">
                        <a href="http://localhost:8080/#/aktDetails/predlozeni/{@ref_akt}#Tacka{@ref_tacka}"> <xsl:value-of select="text()"/></a>
                    </xsl:when>
                    <xsl:when test="@ref_stav">
                        <a href="http://localhost:8080/#/aktDetails/predlozeni/{@ref_akt}#Stav{@ref_stav}"> <xsl:value-of select="text()"/></a>
                    </xsl:when>
                    <xsl:when test="@ref_clan">
                        <a href="http://localhost:8080/#/aktDetails/predlozeni/{@ref_akt}#Clan{@ref_clan}"> <xsl:value-of select="text()"/></a>
                    </xsl:when>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
